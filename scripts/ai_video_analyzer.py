...
def analyze_with_ollama(text, model="qwen2.5:14b"):
    ...
    prompt = f"""Разбей следующий текст на смысловые сегменты для создания видео.

ВАЖНО:
1. "text" - это ТОЧНАЯ ЧАСТЬ ИСХОДНОГО ТЕКСТА (1-3 предложения из оригинала)
2. "search_query" - поисковый запрос для картинки НА АНГЛИЙСКОМ (что должно быть на картинке)
3. НЕ переписывай и НЕ пересказывай текст! Используй ОРИГИНАЛЬНЫЕ предложения из текста!

ПРАВИЛА для "text":
- Бери ОРИГИНАЛЬНЫЕ предложения из текста
- Разбивай по смыслу (1-3 предложения на сегмент)
- Сохраняй ВСЕ слова из оригинала
- Покрывай ВЕСЬ текст без пропусков

ПРАВИЛА для "search_query":
- Описывай ВИЗУАЛЬНОЕ содержание (что показать на картинке)
- Используй конкретные объекты, персонажей, места
- НЕ используй абстрактные понятия (hatred, goal, plan)
- Каждый запрос должен быть УНИКАЛЬНЫМ

ПРИМЕРЫ:
✅ text: "Итачи Учиха с детства видел войну и слишком рано понял, какой ценой даётся мир."
✅ search_query: "itachi uchiha child war destroyed village background"

✅ text: "Клан Учиха, чувствуя себя отстранённым и несправедливо подозреваемым, начал тайно готовить переворот."
✅ search_query: "uchiha clan meeting secret room dark atmosphere"

❌ text: "Итачи видит войну." (НЕПРАВИЛЬНО - это пересказ!)
❌ search_query: "clan plan" (НЕПРАВИЛЬНО - слишком абстрактно!)

Верни ответ СТРОГО в JSON формате:
[
  {{
    "text": "оригинальная часть текста",
    "search_query": "visual description in english",
